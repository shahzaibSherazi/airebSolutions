...
//     <>
//       <StyleInjector />
//       <div className="w-full bg-[#02070F]  text-white">
...
//             }}
//             className="heros_height relative flex items-center justify-center py-16 lg:py-24">
//             <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
//               <div>
...
//                     post={relatedPost}
//                     onClick={() =>
//                       navigate(`/resources/blogs/${relatedPost._id}`)
//                     }
//                   />
//                 ))}
//               </div>
...
// ── In-memory cache (persists across navigations within the session) ──
const blogCache = new Map<string, Blog>();
const relatedCache = new Map<string, Blog[]>();
...
// ── Rich-text + shimmer styles injected once ──────────────────────────
const RICH_TEXT_STYLES = `
  @keyframes shimmer {
  ...
  .ql-content strong, .ql-content b { color: #ffffff; font-weight: 600; }
  .ql-content em, .ql-content i { color: rgba(255,255,255,0.8); font-style: italic; }
  .ql-content a {
    color: #4285F4;
    text-decoration: none;
    border-bottom: 1px solid rgba(66,133,244,0.3);
  ...
  .ql-content a:hover { border-color: #4285F4; }
  .ql-content blockquote {
    border-left: 3px solid #4285F4;
    margin: 1.5rem 0;
    padding: 0.75rem 1.25rem;
  ...
  .ql-content blockquote p { color: rgba(255,255,255,0.65); font-style: italic; margin: 0; }
  .ql-content pre, .ql-content code {
    font-family: 'Outfit', Courier, monospace;
    background: rgba(255,255,255,0.06);
    border: 1px solid rgba(255,255,255,0.1);
  ...
  .ql-content pre { padding: 1rem 1.25rem; overflow-x: auto; margin: 1rem 0; display: block; }
  .ql-content code { padding: 2px 6px; font-size: 13px; }
  .ql-content img { max-width: 100%; border-radius: 8px; margin: 1rem 0; }
  .ql-content hr { border: none; border-top: 1px solid rgba(66,133,244,0.2); margin: 2rem 0; }
  .ql-content .ql-align-center  { text-align: center; }
  .ql-content .ql-align-right   { text-align: right; }
  .ql-content .ql-align-justify { text-align: justify; }
  .ql-content .ql-indent-1 { padding-left: 1.5rem; }
...
function LazyImage({
  ...
  return (
    <div style={{ position: "relative", overflow: "hidden", ...wrapperStyle }}>
      {/* Shimmer shown until image is loaded */}
      {!loaded && (
        <div
      ...
      <div className="container space-y-10">
        ...
        <div className="space-y-4 max-w-3xl">
          {[100, 83, 66, 100, 75].map((w, i) => (
            <div key={i} className={`h-4 w-[${w}%] bg-white/10 rounded`} />
          ))}
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10">
          {[1, 2, 3].map((i) => (
...
export default function BlogDetail() {
  ...
  useEffect(() => {
    ...
    if (blogCache.has(id)) {
      setPost(blogCache.get(id)!);
      setRelatedPosts(relatedCache.get(id) ?? []);
    ...
    Promise.all([blogAPI.getBlog(id), blogAPI.getBlogs(1, 10, "published")])
      .then(([res, allBlogs]) => {
        if (!res.success) {
          setError("Blog post not found.");
          return;
        ...
        const fetchedPost: Blog = res.blog;
        const related: Blog[] = (allBlogs.blogs ?? []).filter(
          (p: Blog) => p._id !== id && fetchedPost.relatedIds?.includes(p._id),
        );

        // Store in cache
        blogCache.set(id, fetchedPost);
        relatedCache.set(id, related);

        setPost(fetchedPost);
        setRelatedPosts(related);

        // Small delay so skeleton unmounts cleanly before fade-in starts
  ...
  return (
    <>
      <StyleInjector />
      <div
        className="w-full bg-[#02070F] text-white"
        style={{
            ...
            <div
              ...
              <main>
                {hasRichContent ? (
                  <div
                    ref={contentRef}
                    className="ql-content blog-fade-in"
                    dangerouslySetInnerHTML={{ __html: post.contentHtml || "" }}
                  />
                ) : (
                  <div className="space-y-12 blog-fade-in">
                    {sections.map((section: any) => (
                      <section
                        key={section.id}
                        id={section.id}
                          ...
                          ? tocItems.map((item) => (
                              <button
                                ...
                                <span
                                  className="mt-1.5 shrink-0 rounded-full transition-all duration-200"
                                  style={{
                                    width: item.level === "h2" ? "6px" : "4px",
                                    height: item.level === "h2" ? "6px" : "4px",
                                    minWidth:
                                      item.level === "h2" ? "6px" : "4px",
                                    background:
                                      activeSection === item.id
