...
function SmallCard({
  ...
  return (
    <div onClick={onReadMore} className="flex flex-col h-full group">
      <div
      ...
      {/* ✅ KEY FIX: flex-1 makes this grow to fill remaining card height */}
      <div
        ...
        {/* ✅ mt-auto pushes the footer to the bottom regardless of title length */}
...
export default function AllBlogs() {
  ...
  const filtered =
    activeTab === "All Blogs"
      ? blogs
      : blogs.filter((p) => p.category === activeTab);
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              {largeCards.map((post) => (
                <SmallCard
                  ...
                  post={post}
                  onReadMore={() => navigate(`/resources/blogs/${post._id}`)}
                />
              ))}
            </div>
            ...
            {Array.from({ length: totalPages }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-sm font-medium transition ${currentPage === page ? "bg-primary text-white shadow-md shadow-primary/40" : "bg-white/5 text-white/60 hover:bg-white/10"}`}>
                  {page}
                </button>
              );
            })}
