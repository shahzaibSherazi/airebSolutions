...
import { AlertCircle, Loader2, Trash2, Edit2, Eye, Plus } from "lucide-react";
...
export default function BlogsManagement() {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setBlogError(error.response?.data?.message || "Failed to delete blog");
    }
  };
  ...
  return (
    <AdminLayout
      ...
      subtitle="Manage and publish your blog posts.">
      {/* Toolbar */}
      ...
      {blogError && (
        <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-lg flex items-start gap-3">
          <AlertCircle
            ...
            size={18}
          />
          <p className="text-red-400 text-sm">{blogError}</p>
        </div>
      )}
              ...
              <tbody className="divide-y divide-primary/10">
                {blogs.map((blog) => (
                  <tr
                    ...
                    <td className="px-4 py-3 text-white/70">{blog.category}</td>
